...
test('adds to .entries', (t) => {
  ...
  t.equal(response.entries.length, 1)
...
test('adds to .referrals', (t) => {
  ...
  t.equal(response.referrals.length, 1)
  t.equal(response.referrals[0], 'ldap://example.com/')
})
...
test('passes along ldap errors', (t) => {
  ...
  response.on('ldaperror', (error) => {
    t.type(error, Error)
    t.equal(error.message, 'LDAP_OPERATIONS_ERROR')
  })
